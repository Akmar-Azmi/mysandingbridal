...
@section('content')
    ...
    <section class="py-16 bg-white">
        <div class="container mx-auto grid md:grid-cols-2 gap-12 px-6">
            <div>
                ...
                <p class="text-gray-600 mb-6">Lorem ipsum dolor sit amet. Sed aperiam amet est voluptate corrupti a quaerat debitis sed officiis velit ut saepe galisum et repellat voluptates...</p>
                <a href="#" class="inline-block border border-black px-5 py-2 rounded-full text-sm hover:bg-black hover:text-white transition">Read More</a>
            </div>
            <div class="grid grid-cols-2 gap-4">
                @foreach(['why1.jpg', 'why2.jpg', 'why3.jpg'] as $img)
                    <img src="{{ asset('images/homepage/' . $img) }}" class="rounded-md shadow" alt="">
                @endforeach
            </div>
        </div>
    </section>
    ...
    <section class="bg-[#D8B57F] py-16">
        <h2 class="text-center text-2xl font-semibold mb-8">Services</h2>
        <div class="container mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 px-6">
            @foreach(['services1.jpg', 'services2.jpg', 'services3.jpg', 'services4.jpg'] as $img)
                <div class="text-center">
                    <img src="{{ asset('images/homepage/' . $img) }}" class="rounded shadow mb-2" alt="">
    ...
    {{-- Stats --}}
    <section class="bg-white py-12 text-center grid grid-cols-1 md:grid-cols-3 gap-6">
        @foreach(['Event', 'Client', 'Client'] as $type)
            <div class="text-3xl font-bold text-black">+500 <div class="text-lg font-normal">{{ $type }}</div></div>
        @endforeach
    </section>
    ...
    <section class="bg-[#D8B57F] py-16">
        ...
        <div class="container mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 px-6">
            @foreach(range(1, 8) as $i)
                <img src="{{ asset("images/homepage/gallery{$i}.jpg") }}" class="rounded-md shadow" alt="">
            @endforeach
        </div>
        <div class="text-center mt-6">
            <a href="{{ route('gallery') }}" class="inline-block border border-black px-5 py-2 rounded-full text-sm hover:bg-black hover:text-white transition">View More</a>
        </div>
    </section>
    ...
    {{-- Call to Action --}}
    <section class="bg-[#D8B57F] text-center py-16">
        <h2 class="text-xl font-medium text-white">Let’s Turn Your Love Story Into an Unforgettable Celebration</h2>
        <p class="text-white mb-6">– Schedule a Chat With Us Today! –</p>
        <a href="{{ route('book') }}" class="bg-yellow-400 hover:bg-yellow-500 text-black font-semibold px-6 py-2 rounded-full border border-black">Book Appointment</a>
    </section>
@endsection
